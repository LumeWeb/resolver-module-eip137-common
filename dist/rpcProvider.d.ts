...
export default class RpcProvider extends ethers.providers.BaseProvider {
    ...
    static hexlifyTransaction(transaction: ethers.providers.TransactionRequest, allowExtra?: {
        [key: string]: boolean;
    }): {
...
declare class RpcSigner extends ethersAbstractSigner.Signer {
    ...
    sendUncheckedTransaction(transaction: ethersProperties.Deferrable<ethersAbstractProvider.TransactionRequest>): Promise<string>;
    signTransaction(transaction: ethersProperties.Deferrable<ethersAbstractProvider.TransactionRequest>): Promise<string>;
    sendTransaction(transaction: ethersProperties.Deferrable<ethersAbstractProvider.TransactionRequest>): Promise<ethersAbstractProvider.TransactionResponse>;
    signMessage(message: ethers.Bytes | string): Promise<string>;
    _legacySignMessage(message: ethers.Bytes | string): Promise<string>;
    _signTypedData(domain: ethersAbstractSigner.TypedDataDomain, types: Record<string, ethersAbstractSigner.TypedDataField[]>, value: Record<string, any>): Promise<string>;
